...
TH1 * h1charge = new TH1D("h1charge", ";Charge (fC);Entries",3000,0,3000);
TH1 *  h1MaxAmp = new TH1D("h1MaxAmp", "Signal;Amplitude (mV);Entries",8000,-1000,1000);
...
h1Type GetBaseLine(float * tempBuffer, const UInt_t baseLineSample=100){
  ...
  for (UInt_t i = 0; i < baseLineSample; i++){
    gr ->SetPoint(i, i, tempBuffer[i]*ampRes);
  }
...
double GetVmax(TGraph * gr, double  WStart=0 ,   double  WEnd = 204.8)
{
  double vmax=0;
